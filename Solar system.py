...
def drawPolygon(canvas,pts,color,axis=False):
    ...
    if axis==True:
        C = getCenter(pts)
        getline(canvas, C[0,0],C[0,1], pts[0,0],pts[0,1], color=(255, 128, 128))
...
def main():
    ...
    while True:
        ...
        Rocket_p = cvtto3mat(Rocket_p,ngon)
        
        Sun = makeTmat(530,400) @ Sun_p
        Sun = rettomat(Sun)
        drawPolygon(canvas,Sun,color)
        
        Venus = makeTmat(530,400)@ makeRmat(-(v_rev)) @ makeTmat(160,0) @ makeRmat(v_rot)@Venus_p
        Venus = rettomat(Venus)
        drawPolygon(canvas,Venus,color,axis=True)
    
        Earth = makeTmat(530,400)@ makeRmat(e_rev) @ makeTmat(260,0)@ makeRmat(e_rot) @Earth_p
        Earth = rettomat(Earth)

        Moon = makeTmat(530,400)@ makeRmat(e_rev) @ makeTmat(260,0)@ makeRmat(e_rot) @makeRmat(m_rot)@makeTmat(130,0)@makeRmat(m_rot)@Moon_p
        Moon = rettomat(Moon)
        
        drawPolygon(canvas,Earth,color,axis=True)
        drawPolygon(canvas,Moon,color,axis=True)
        ctoc(canvas,Earth,Moon)
        
        Rocket = makeTmat(530,400)@ makeRmat(e_rev) @ makeTmat(260,0)@ makeRmat(e_rot) @makeRmat(m_rot)@makeTmat(130,0)@makeRmat(m_rot)@makeRmat(r_rot)@makeTmat(70,0)@makeRmat(-r_rot)@Rocket_p
        Rocket = rettomat(Rocket)
        drawPolygon(canvas,Rocket,color,axis=True)
        ctoc(canvas,Rocket,Moon)
